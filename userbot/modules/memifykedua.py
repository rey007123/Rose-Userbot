...
async def mim(event):
    ...
    if not event.reply_to_msg_id:
        await event.edit(
            "Mohon Balas Ke Gambar Ketik `.mmf3 'Teks Atas' ; 'Teks Bawah'` "
        )
        return
...
async def draw_meme_text(image_path, text):
    ...
    m_font = ImageFont.truetype(
        "userbot/utils/styles/ChunkFive-Regular.otf", int((95 / 730) * i_width)
    )
    if ";" in text:
...
@register(outgoing=True, pattern=r"^\.mmf4(?: |$)(.*)")
async def mim(event):
    if event.fwd_from:
        return
    if not event.reply_to_msg_id:
        await event.edit(
            "Mohon Balas Ke Gambar Ketik `.mmf4 'Teks Atas' ; 'Teks Bawah'` "
        )
        return
    reply_message = await event.get_reply_message()
    if not reply_message.media:
...
async def draw_meme_text(image_path, text):
    ...
    i_width, i_height = img.size
    m_font = ImageFont.truetype(
        "userbot/utils/styles/Contenderfreeversion-eZ596.otf", int((95 / 730) * i_width)
    )
    if ";" in text:
        upper_text, lower_text = text.split(";")
    else:
...
@register(outgoing=True, pattern=r"^\.mmf5(?: |$)(.*)")
async def mim(event):
    if event.fwd_from:
        return
    if not event.reply_to_msg_id:
        await event.edit(
            "Mohon Balas Ke Gambar Ketik `.mmf5 'Teks Atas' ; 'Teks Bawah'` "
        )
        return
    reply_message = await event.get_reply_message()
    if not reply_message.media:
...
async def draw_meme_text(image_path, text):
    ...
    i_width, i_height = img.size
    m_font = ImageFont.truetype(
        "userbot/utils/styles/KaushanScript-Regular.otf", int((95 / 730) * i_width)
    )
    if ";" in text:
        upper_text, lower_text = text.split(";")
    else:
